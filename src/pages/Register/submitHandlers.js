...
export async function submitUser(values) {
  ...
  const response = await axios.post('https://api.hagznow.com/auth/register', payload);
...
export async function submitOwner(values) {
  ...
  const response = await axios.post('https://api.hagznow.com/auth/register/owner', formData, {
