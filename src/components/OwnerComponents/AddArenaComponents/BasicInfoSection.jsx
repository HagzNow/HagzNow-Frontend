...
const BasicInfoSection = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await axios.get('https://api.hagznow.com/categories');
  ...
  return (
    <div className="border dark:border-gray-700 rounded-xl p-6 bg-white dark:bg-gray-800 transition-colors">
      <h3 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">{t('basicInfo')}</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <ErrorMessage name="categoryId" component="div" className="text-red-500 dark:text-red-400 text-sm mt-1" />
        </div>
      </div>
    </div>
  );
